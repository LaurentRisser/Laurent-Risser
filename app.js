...
function getDomForCommand(command) {
  ...
  if (commandObj === undefined) {
    html = `'${
      command.split(" ")[0]
    }' is not a valid command. Type 'help' to see all available commands.`;
  } else {
  ...
  switch (command) {
    ...
    case "light":
      if (document.body.classList.length === 0)
        return "Your are already in the bright mode";
      setDarkMode(false);
      return "Your are now in the bright mode";
    case "dark":
      if (document.body.classList.length === 1)
        return "Your are already in the dark mode";
      setDarkMode(true);
      return "Your are now in the dark mode";
    case "get cv":
      getCV();
